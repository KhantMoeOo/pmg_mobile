    ...
    try {
      ...
      Sharef.getOdooClientInstance().then((value) async {
        ...
        OdooResponse res = await odoo.create('purchase.requisition', {
          ...
          // 'ref_no': refno,
          // 'zone_id': zoneId,
          'request_person_id': requestPerson,
          'department_id': departmentId,
          'location_id': locationId,
          // 'invoice_id': invoiceId,
          // 'priority': priority,
          'order_date': orderdate,
          'required_date': scheduledDate,
          // 'desc': description,
        });
  ...
  updateMaterialRequisitionStatusData(ids, state) {
    ...
    try {
      ...
      Sharef.getOdooClientInstance().then((value) async {
        ...
        odoo.setSessionId(value['session_id']);
        OdooResponse res =
            await odoo.write('purchase.requisition', [ids], {'state': state});
        if (res.getResult() != null) {
          print('result');
  ...
  callActionConfirm(id) async {
    ...
    try {
      ...
      Sharef.getOdooClientInstance().then((value) async {
        ...
        odoo.setSessionId(value['session_id']);
        OdooResponse res =
            await odoo.callKW('purchase.requisition', 'action_confirm', [id]);
        if (!res.hasError()) {
